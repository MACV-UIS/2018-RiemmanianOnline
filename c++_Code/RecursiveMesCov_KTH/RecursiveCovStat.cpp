                        ...
                        vector<string> vec_activities, int NFEATURES, int num_mean, float init_alpha)
{
    ...
    if(info_traj[info_traj.size()-2].compare("ImpTraj")==0 || info_traj[info_traj.size()-2].compare("DenseTraj")==0
       || info_traj[info_traj.size()-1].compare("improved")==0 )
    {
        num_scal_traj =1;
    }
...
//        string path_textTrajectories_k = str_path_trajectories + vec_videos[k] + ".scale";
        ...
        infoVideoFromTrajecAllScales(path_textTrajectories_k, numTotalFrames_k,
                                     heightFrame_k, widthFrame_k, num_tot_traj_k, num_scal_traj);
                                      ...
                                      vector<string> vec_activities, int NFEATURES, int num_mean, float init_alpha)
{
    ...
    int num_scal_traj = 0;
    if(info_traj[info_traj.size()-2].compare("ImpTraj")==0 ||
        info_traj[info_traj.size()-2].compare("DenseTraj")==0 || info_traj[info_traj.size()-1].compare("improved")==0)
    {
        num_scal_traj =1;
    }
    else if(info_traj[info_traj.size()-2].compare("EnstaTraj")==0)
...
cout<<endl<<"SIZE: "<<vec_videos.size()<<endl<<endl;
    ...
    {
        cout<< k<< " )  processing video: "<< vec_videos[k]  << endl;
        //string path_textTrajectories_k = str_path_trajectories + vec_videos[k] + ".scale"; //code posdoc
        string path_textTrajectories_k = str_path_trajectories + vec_videos[k] + ".txt";
        cout<< "22 oct: "<<path_textTrajectories_k<< endl;
        int numTotalFrames_k, heightFrame_k, widthFrame_k, num_tot_traj_k;
        infoVideoFromTrajecAllScales(path_textTrajectories_k, numTotalFrames_k,
                                     heightFrame_k, widthFrame_k, num_tot_traj_k, num_scal_traj);
...
///-----------------escribir ejemplos cada 1/alpha value
            ...
            if(frame_jump>= cont_tot_DifCov){
                frame_jump = cont_tot_DifCov-1;
                }

//            cout<< " frame jump "<< frame_jump << endl;
//            cin.ignore();
            ...
            for(int iter_jump= frame_jump; iter_jump < cont_tot_DifCov; iter_jump = iter_jump+ (frame_jump/2) )
            {
                ...
                for(int i=0; i< NFEATURES; i++)
                {
                    for(int j=i; j< NFEATURES; j++)
                    {
                        outdata_mean_var<< cont_P << ":"<<array_mean[iter_jump][i][j] << " ";
                        outdata_conc_all<< cont_P << ":"<<array_mean[iter_jump][i][j] << " ";
                        cont_P++;
                    }
                }
                ...
                for(int i=0; i< NFEATURES; i++)
                {
                    for(int j=i; j< NFEATURES; j++)
                    {
                        outdata_mean_var<< cont_P << ":"<<array_var[iter_jump][i][j] << " ";
                        outdata_conc_all<< cont_P << ":"<<array_var[iter_jump][i][j] << " ";
                        cont_P++;
                    }
                }
