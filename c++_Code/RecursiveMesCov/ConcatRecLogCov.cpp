                    ...
                    vector<string> vec_activities, int NFEATURES, int num_mean, float init_alpha)
{
    ...
    for(int k=0; k< vec_videos.size(); k++)
    {
        ...
        //string path_textTrajectories_k = str_path_trajectories + vec_videos[k] + ".scale0"; //para UT
        ...
        for(int frame_index=3; frame_index< numTotalFrames_k; frame_index++)
        {
            ...
            cout << " numero de framexxxxxx: "<< frame_index << " numTotalFrames_k: "<< numTotalFrames_k << " trajectories used "<< traj_used << endl;
            if(traj_used>100)
            {
        ...
        cout<<"va a empezar con la info nueva"<<endl;
        for(int i=0; i< numTotalFrames_k; i++){

                cout<<"frame index: "<< arr_info_first[i][0]<< " ";
                cout<<" val dist: "<< arr_info_first[i][1]<< " ";
                cout<<"cont_frameCov"<< arr_info_first[i][2]<< endl;
        }
        cout<< "termino"<< endl;
...
void Framelevel_Rec_Log_CoV(vector<string>  vec_videos, string name_file,  string str_path_trajectories,
                    vector<string> vec_activities, int NFEATURES, int num_mean, float init_alpha)
{
    /// The idea is to run partial recognition at each 10%
    ///famarcar_15Nov
    // The experiments were carried out with LED
    ofstream outdata_mean((name_file + "mean.txt").c_str(), fstream::out); //1 means numero de divisiones del video
    outdata_mean.precision(10);
